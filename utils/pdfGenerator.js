...
export function createPdfDefinition(formData, includePrice = false) {
  ...
  const docDefinition = {
    content: [
      ...
      {
        text: `Submitted on: ${submittedAt || new Date().toLocaleDateString()}`,
        style: "date",
      },

      // Include price if needed (for admin version)
      includePrice
        ? { text: `Price: $499`, style: "price", margin: [0, 0, 0, 15] }
        : {},
...
function generateProjectsContent(projects) {
  ...
  return projects.map((project, index) => {
    return {
      stack: [
        {
          text: `Project ${index + 1}: ${project.title || "Untitled"}`,
          style: "subsubheader",
        },
        {
          table: {
...
function generateExperienceContent(experience) {
  ...
  return experience.map((exp, index) => {
    return {
      stack: [
        {
          text: `${exp.company || "Unknown Company"} - ${
...
function generateEducationContent(education) {
  ...
  return education.map((edu, index) => {
    return {
      stack: [
        {
          text: `${edu.institution || "Unknown Institution"} - ${
...
export async function generatePdfBuffer(formData, includePrice = false) {
  ...
  return new Promise((resolve, reject) => {
    try {
      // Using a different approach for server-side environments
